<cfscript>
  ...
  param ATTRIBUTES.variable = "build_from_json_result";
  
  // Various Checks prior to executing the code
  if (ATTRIBUTES.file EQ "") { throw(message="FILE is required for #ThisTag.name#"); }
  if (ATTRIBUTES.json EQ "") { throw(message="JSON is required for #ThisTag.name#"); }
...
>
  <cfscript>
    ...
    */
    ...
    LOCAL.struct_content = DeserializeJSON(ATTRIBUTES.json);

    // Parse through JSON to populate workbook
    LOCAL.workbook = populateFromJSON(LOCAL.workbook, ATTRIBUTES.json);
